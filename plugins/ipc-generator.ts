...
function generateIpc(options: IpcGeneratorOptions) {
    ...
    try {
        ...
        currentTypesContent = typesSkeleton;
    }
    ...
    if (typesSectionMatch) {
        ...
        const entryRegex = /^\s*(?:"([^"]+)"|'([^']+)'|([A-Za-z0-9_$]+))\s*:\s*([\s\S]*?);/gm;
        let typeEntryMatch;
    ...
    const typesEol = getEol(currentTypesContent);
    const typesReplacement = sortedChannelNames
        ...
        .join(typesEol);
    const newTypesContent = currentTypesContent.replace(
        /(\/\/ IPC_HANDLERS_START).*?(\/\/ IPC_HANDLERS_END)/s,
        `$1${typesEol}${typesReplacement}${typesEol}    $2`,
    );

    if (newTypesContent !== currentTypesContent) {
